...
from app.rag.query_all import query_bns
import time
import random
...
class SectionsCharged(BaseModel):
    ...
    source: str = Field(
        description="Source information including page number, PDF document name, and source URL from the BNS document (format: Page X, Document: [pdf_name], Source URL: [source_url])"
    )

class BnsLegalMapping(BaseModel):
...
def _merge_duplicate_sections(section_number: str, duplicate_sections: List[SectionsCharged]) -> SectionsCharged:
    ...
    prompt = f"""
You are an expert in BNS (Bharatiya Nyaya Sanhita) law. You need to merge duplicate entries for the same section number.
...
def bns_legal_mapping(state: WorkflowState) -> dict:
    """
    Map Bharatiya Nyaya Sanhita (BNS) legal provisions to FIR facts.
    """
    print("\n" + "=" * 80)
    print("[bns_legal_mapping] Starting legal mapping...")
    print("=" * 80)
    ...
    print(f"[bns_legal_mapping] FIR content length: {len(pdf_content)} characters")
    print(f"[bns_legal_mapping] FIR preview: {pdf_content[:200]}...")

    # Step 1: Extract actionable legal points from FIR content
    print("\n[bns_legal_mapping] Step 1: Extracting legal points from FIR content...")
    llm_with_structured_output = llm_model.with_structured_output(PointsToBeCharged)
    prompt = f"""
You are an expert in Indian criminal law (Bharatiya Nyaya Sanhita / BNS).
...
- Extract only BNS-related facts (acts, substances, quantities, locations, actions, procedures).
...
"""
    ...
    print("[bns_legal_mapping] Calling LLM to extract legal points...")
    ...
    print(f"[bns_legal_mapping] Extracted {len(points)} legal points:")
    for i, point in enumerate(points, 1):
        print(f"   {i}. {point[:100]}...")

    sections_mapped = []
    print(f"\n[bns_legal_mapping] Step 2: Mapping {len(points)} points to BNS sections...")
    for idx, point in enumerate(points, 1):
        print(f"\n[{idx}/{len(points)}] Processing point: {point[:80]}...")

        # Retrieve relevant BNS sections using query_bns
        print(f"  [{idx}/{len(points)}] Searching FAISS index...")
        results = query_bns(point, k=5)
        ...
        # query_bns returns [{'chunk': {...}, 'score': float}]; chunk structure: section, subsection (may be null), chapter, chapter_heading, content, page_number, source_url, pdf_name
        sections_found = ""
        ...
        llm_with_structured_output = llm_model.with_structured_output(BnsLegalMapping)
        prompt = f"""
You are an expert in BNS (Bharatiya Nyaya Sanhita) law.

Legal Point (from FIR):
{point}

Retrieved BNS Act Text:
{sections_found}

Task:
Identify only the BNS sections that are directly applicable to the legal point from the FIR.

CRITICAL RULES:
1. Use ONLY the retrieved BNS Act text above. Do not add external knowledge, legal interpretations, or assumptions.
...
7. Do NOT interpret or infer connections - use only what is explicitly stated in the retrieved BNS Act text.
...
- source:
  ...
  Example: "Page 15, Document: THE_BHARATIYA_NYAYA_SANHITA_2023.pdf, Source URL: https://www.mha.gov.in/..."
...
"""
    ...
    print("\n[bns_legal_mapping] Step 3: Flattening and deduplicating results...")
    flattened_sections = []
    ...
    print(f"[bns_legal_mapping] Total sections before deduplication: {len(flattened_sections)}")
    print(f"[bns_legal_mapping] Total sections after deduplication: {len(final_sections)}")
    print("=" * 80)

    return {
        "bns_sections_mapped": final_sections
    }
