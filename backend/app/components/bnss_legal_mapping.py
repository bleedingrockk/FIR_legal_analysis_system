...
def _merge_duplicate_sections(section_number: str, duplicate_sections: List[SectionsCharged]) -> SectionsCharged:
    ...
    prompt = f"""
You are an expert in BNSS (Bharatiya Nagarik Suraksha Sanhita) law. You need to merge duplicate entries for the same section number.
...
def bnss_legal_mapping(state: WorkflowState) -> dict:
    ...
    """
    print("\n" + "=" * 80)
    print("[bnss_legal_mapping] Starting legal mapping...")
    print("=" * 80)
    ...
    print(f"[bnss_legal_mapping] FIR content length: {len(pdf_content)} characters")
    print(f"[bnss_legal_mapping] FIR preview: {pdf_content[:200]}...")

    # Step 1: Extract actionable legal points from FIR content
    print("\n[bnss_legal_mapping] Step 1: Extracting legal points from FIR content...")
...
"""
    ...
    print("[bnss_legal_mapping] Calling LLM to extract legal points...")
    ...
    print(f"[bnss_legal_mapping] Extracted {len(points)} legal points:")
    for i, point in enumerate(points, 1):
        print(f"   {i}. {point[:100]}...")

    sections_mapped = []
    print(f"\n[bnss_legal_mapping] Step 2: Mapping {len(points)} points to BNSS sections...")
    for idx, point in enumerate(points, 1):
        print(f"\n[{idx}/{len(points)}] Processing point: {point[:80]}...")

        # Retrieve relevant BNSS sections using query_bnss
...
"""
    ...
    print("\n[bnss_legal_mapping] Step 3: Flattening and deduplicating results...")
    flattened_sections = []
    ...
    print(f"[bnss_legal_mapping] Total sections before deduplication: {len(flattened_sections)}")
    print(f"[bnss_legal_mapping] Total sections after deduplication: {len(final_sections)}")
    print("=" * 80)
