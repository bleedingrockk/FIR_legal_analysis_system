...
def _merge_duplicate_sections(section_number: str, duplicate_sections: List[SectionsCharged]) -> SectionsCharged:
    ...
    prompt = f"""
You are an expert in BSA (Bharatiya Sakshya Adhiniyam) law. You need to merge duplicate entries for the same section number.
...
def bsa_legal_mapping(state: WorkflowState) -> dict:
    ...
    """
    print("\n" + "=" * 80)
    print("[bsa_legal_mapping] Starting legal mapping...")
    print("=" * 80)
    ...
    print(f"[bsa_legal_mapping] FIR content length: {len(pdf_content)} characters")
    print(f"[bsa_legal_mapping] FIR preview: {pdf_content[:200]}...")

    # Step 1: Extract actionable legal points from FIR content
    print("\n[bsa_legal_mapping] Step 1: Extracting legal points from FIR content...")
...
"""
    ...
    print("[bsa_legal_mapping] Calling LLM to extract legal points...")
    ...
    print(f"[bsa_legal_mapping] Extracted {len(points)} legal points:")
    for i, point in enumerate(points, 1):
        print(f"   {i}. {point[:100]}...")

    sections_mapped = []
    print(f"\n[bsa_legal_mapping] Step 2: Mapping {len(points)} points to BSA sections...")
    for idx, point in enumerate(points, 1):
        print(f"\n[{idx}/{len(points)}] Processing point: {point[:80]}...")

        # Retrieve relevant BSA sections using query_bsa
        ...
        # query_bsa returns [{'chunk': {...}, 'score': float}]; chunk structure: section, subsection (may be null), chapter, chapter_heading, content, page_number, source_url, pdf_name
        sections_found = ""
...
Retrieved BSA Act Text:
{sections_found}

Task:
Identify only the BSA sections that are directly applicable to the legal point from the FIR.

CRITICAL RULES:
1. Use ONLY the retrieved BSA Act text above. Do not add external knowledge, legal interpretations, or assumptions.
...
7. Do NOT interpret or infer connections - use only what is explicitly stated in the retrieved BSA Act text.
...
"""
    ...
    print("\n[bsa_legal_mapping] Step 3: Flattening and deduplicating results...")
    flattened_sections = []
    ...
    print(f"[bsa_legal_mapping] Total sections before deduplication: {len(flattened_sections)}")
    print(f"[bsa_legal_mapping] Total sections after deduplication: {len(final_sections)}")
    print("=" * 80)
